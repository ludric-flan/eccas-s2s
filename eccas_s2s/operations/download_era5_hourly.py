...
2026-09-19): **one request per year**, sent one after the other, for the hourly
...
def run(config: str, first_year: int, last_year: int, keep_hourly: bool = False) -> RunContext:
    import cdsapi
    ...
    with RunContext(cfg, step="download_era5_hourly") as ctx:
        ...
        ctx.record_parameter("method", "1 request per year, sequential; daily mean/max/min computed locally")
        failures = {}
        for year in range(first_year, last_year + 1):
            ...
            if daily_path.exists():
                ctx.log.info("%d déjà traité : %s", year, daily_path.name)
                ctx.record_output(daily_path, role="era5_t2m_daily", year=year)
                continue
            grib = out_dir / f"era5_t2m_hourly_{year}.grib"
            try:
                if not grib.exists():
                    ctx.log.info("%d : requête CDS (T2m horaire, mois %d–%d)", year, months[0], months[-1])
                    part = grib.with_suffix(".part")
                    for k in range(1, MAX_ATTEMPTS + 1):
                        try:
                            cdsapi.Client(quiet=True).retrieve(DATASET, build_request(year, months), str(part))
                            break
                            ...
                            time.sleep(120 * k)
                    part.replace(grib)
                ctx.record_input(grib, role="era5_t2m_hourly", year=year)
...
def main(argv=None):
    ...
    run(args.config, args.years[0], args.years[1], args.keep_hourly)
